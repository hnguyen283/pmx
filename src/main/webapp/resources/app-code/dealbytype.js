...
var sizeOfDeal = 1;
var currentTypeId = $(".btn_TypeId").val();
				...
				function() {
					...
					$("#bt_changeSortNew").click(
							function(e) {
								...
								getDeals(toggleSort, "getbybegintime",
										"#bodyNew", 0, currenPoint, currentTypeId);
					...
					$("#bt_changeSortNearDeadLine").click(
							function(e) {
								...
								getDeals(toggleSort, "getbyendtime",
										"#bodyNearDeadline", 0, currenPoint,currentTypeId);
								$("#preloading").addClass("loaded");
							});
					...
					$(document).ready(
							function() {
								$("#bodyNew").html('');
								getDeals(toggleSort, "getbybegintime",
										"#bodyNew", 0, numberItemGet, currentTypeId);
								console.log(currentTypeId);
								$.ajax({
									async : false,
									url : base_URL + "/api/dealbytype/getnumberdeal/" + currentTypeId,
									type : "GET",
					...
					function getDeals(toggle, linkDeal, temp, startPoint,
						...
						if (toggle) {
							...
							$.ajax({
								async : false,
								url : base_URL + "/api/dealbytype/" + linkDeal + "/" + typeId,
								type : "GET",
								cache : false,
								data : {
									"sort" : "ASC",
					...
					function processing(json, isPage1) {
						...
						if (json.length > 0) {
							for (var i = 0; i < json.length; i++) {
								...
								var imagePanel = '  <img class="activator responsive-img" src="'
										+ base_URL
										+ '/resources/image/'
										+ json[i].deal_id
										+ '/'
								...
								var item = '<div class="col s12 '
										+ cardSizeClass
										+ '"> <div class="card medium" id=card_'
										+ json[i].deal_id
										+ '> '
										...
										+ '</div> </div> <div class="card-reveal"> <span class="card-title grey-text text-darken-4">'
										+ json[i].tbl_item.item_name
