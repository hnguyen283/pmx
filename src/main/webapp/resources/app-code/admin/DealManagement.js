...
//	---------------------------------------------------------------------------------------------
	...
	$("#form_tableDeal").submit(function(e){
		e.preventDefault();
		var formData = $(this).serialize();
		$.ajax({
			url: base_URL+"/api/deal/delete",
			type: "POST",
			data: formData,
			success: function(data){
				var json = JSON.parse(data);
	...
	$("#btn_add").click(function(e) {
		...
		var modal = $("#modal_deal");
		var form_order = modal.find("#form_deal");
	...
	$(document).on("click", ".btn_edit", function(e){
		...
		$.ajax({
			...
			success: function(data){
				var json = JSON.parse(data);
				var modal_deal = $("#modal_deal");
				var form_deal = $("#form_deal");
				for(var key in json){
					if (json.hasOwnProperty(key)) {
						if (json[key] != undefined && json[key] != "") {
							form_deal.find("input[name="+ key+ "]").val(json[key]);
							console.log("input[name="+ key+ "]" + " / " + form_deal.find("input[name="+ key+ "]").val());
						}
					}
				}
	...
	$("#form_deal").submit(function(e){
		...
		if(dealId == undefined || dealId == ""){
			link = base_URL+"/api/deal/create";
		}else {
			$(this).find("input[name=deal_id]").prop('disabled', false);
			link = base_URL+"/api/deal/update";
		}
		var formData = $(this).serialize();
		$(this).find("input[name=deal_id]").prop('disabled', true);
		$.ajax({
			url: link,
			type: "POST",
			data: formData
			...
			var json = JSON.parse(data);
			console.log(json);
			var uploadFiles =  document.getElementById("input_files_upload");
			if(json.result == "success"){				
				$.ajax({
					url: base_URL+"/api/deal/upload/"+json.detail.deal_id,
					type: "POST",
		            data: new FormData(document.getElementById("form_files_upload")),
		            processData: false,
		            contentType: false
				}).done(function(data2){
	...
	function getDeal(number_page){
		if(number_page == 1){
			...
			reloadDeal((number_page - 1)* sizePagination);
		}
	}
					...
					var row = '<tr>'+
									'<td>'+
										'<div class="input-field">'+
											'<input type="checkbox" id="cb_'+json[i].deal_id+'"'+
												'name="listOrderId" value="'+json[i].deal_id+'"> <label '+
												'for="cb_'+json[i].deal_id+'"></label>'+
										'</div>'+
									'</td>'+
									'<td>'+json[i].deal_owner+'</td>'+
									'<td>'+json[i].deal_begin+'/'+json[i].deal_end+'</td>'+
									'<td>'+json[i].tbl_item.item_name+'</td>'+
									'<td>'+json[i].deal_price+'</td>'+
									'<td>'+json[i].deal_status+'</td>'+
									'<td>'+
										'<button class="btn wave-effect wave-light btn_edit"'+
										'value="'+json[i].deal_id+'" type="button">Edit'
										+ '<i class="fa fa-pencil right"></i></button>'+
									'</td>'+
								'</tr>';
			...
			success: function(data){
				...
				for(var i=0;i<json.length;i++){
					var row = '<tr>'+
									...
									'<td>'+json[i].deal_status+'</td>'+
									'<td>'+
										'<button class="btn wave-effect wave-light red btn_delete"'+
										'value="'+json[i].deal_id+'" type="button">Delete</button>'+
									'</td>'+
								'</tr>';
					table_Deal.find("tbody").append(row);
				}
